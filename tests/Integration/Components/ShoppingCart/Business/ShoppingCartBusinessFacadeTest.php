...
class ShoppingCartBusinessFacadeTest extends KernelTestCase
{
    ...
    {
        $res = $this->shoppingCartBusinessFacade->getCart(1);

        self::assertSame(334, $res[0]->itemId);
        self::assertSame('Gregor Kobel', $res[0]->name);

        self::assertSame(176268, $res[1]->itemId);
        self::assertSame('Silas Ostrzinski', $res[1]->name);
    }
    ...
    public function testSaveItemToCartIncreaseQuantityOfExistingItem(): void
    {
        ...
        (
            quantity: 10, itemId: 334, userId: 1, id: 0
        );

        $this->shoppingCartBusinessFacade->saveItemToCart($shoppingCartDTO);

        $res = $this->shoppingCartRepository->findByUserId(1);

        self::assertSame(10, $res[0]->quantity);
        self::assertSame(334, $res[0]->itemId);
        self::assertSame(1, $res[0]->userId);
    }
